...
import { supportedFormatsAr, supportedFormatsBn, supportedFormatsDe, supportedFormatsEs, supportedFormatsFr, supportedFormatsHi, supportedFormatsJa, supportedFormatsPt, supportedFormatsRu, supportedFormatsZh } from "./supportedFormatsLocale";
...
const supportedFormats = [
  ...
  },
  {
    ...
    description: "A text file format that uses tabs to separate values and newlines to separate records.",
  ...
  },
  {
    extension: ".csv",
    mimeType: "text/csv",
    description: "A text file format that uses commas as a delimiter between values.",
    fileType: "Comma Separated Values"
  },
  {
    extension: ".html",
    mimeType: "text/html",
    description: "HTML is a standard markup language used to structure web pages and content.",
  ...
  },
  {
    ...
    description: "An XML-based file format used for PowerPoint slideshow presentations.",
  ...
  },
  {
    ...
    description: "A standardized XML-based file format widely used in translation and localization software processing.",
...
export const supportedFormatsGroupedByFileType = (locale: Locale) => {
  const supportedFormats = localeSupportedFormatsMap[locale] || localeSupportedFormatsMap['en'];
  const grouped = supportedFormats.reduce((acc, format) => {
    if (!acc[format.fileType]) {
      acc[format.fileType] = { extensions: [], mimeType: format.mimeType, description: format.description };
    }
    acc[format.fileType].extensions.push(format.extension);
    return acc;
  }, {} as Record<string, { extensions: string[], mimeType: string, description: string }>);
